...
export function buildAnalysisReport(input: EngineReportInput): EngineReport {
  ...
  const methodologyFa = [
    ...
    '۳. برای هر نیاز، سه نوع تطابق به ترتیب اولویت بررسی شد: تطابق دستی تأیید‌شده (بازنگی کاربر)، تطابق کامل قالبی (همه قالب‌های لازم موجود)، و تطابق جزئی قالبی (حداقل ۳۰٪ قالب‌ها).',
    ...
    '۶. در پایان، رکورد گپ هر نیاز با امتیاز تطابق، نوع تطابق، شرح فارسی چرایی و زنجیره مرجع (مسیر مالک) ثبت شد. گپ‌های همان درختواره جایگزین شدند اما بازنگی‌های دستی کاربر حفظ و بر نتیجه جدید اعمال گردید (تأیید/رد/تغییر).',
  ];

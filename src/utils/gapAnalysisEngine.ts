...
export function normalizePersianText(input: string | null | undefined): string {
  ...
  // حذف اعراب و اتصال‌های کنترلی
  s = s.replace(/[\u064B-\u065F\u0670\u0640\u200B-\u200F\uFEFF]/g, '');
  // یکسان‌سازی نویسه‌های عربی/فارسی
  s = s.replace(/[يكۀأإآؤئةٱٹڈڑ]/g, (ch) => ARABIC_TO_PERSIAN[ch] || ch);
...
export function buildAnalysisReport(input: EngineReportInput): EngineReport {
  ...
  const methodologyFa = [
    ...
    '۳. برای هر نیاز، سه نوع تطابق به ترتیب اولویت بررسی شد: تطابق دستی تأیید‌شده، تطابق کامل قالبی (همه قالب‌های لازم موجود)، و تطابق جزئی قالبی (حداقل ۳۰٪ قالب‌ها).',
    '۴. اگر تطابق قالبی پیدا نمی‌شد، شباهت فازی ترکیبی محاسبه شد: ۶۰٪ شباهت عنوان (ترکیب لوانشتاین، بای‌گرام و اشتراک واژگان فارسی نرمال‌شده)، ۲۰٪ شباهت جایگاه ساختاری (مسیر والد‌ها) و ۲۰٪ قالب‌ها/نمونه‌های مشترک.',
    '۵. در پایان هر نیاز یک رکورد گپ با امتیاز تطابق، نوع تطابق و شرح فارسی چرایی نتیجه ثبت شد. گپ‌های قبلی همان درختواره جایگزین این تحلیل شدند.',
  ];

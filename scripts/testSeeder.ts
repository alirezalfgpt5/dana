...
console.log("Seeding Knowledge Trees, Nodes, and Issues...");
initDb();
...
if (!period) {
  const stmt = sqlite.prepare("INSERT INTO periods (name, start_date, end_date, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)");
  const info = stmt.run('سال ۱۴۰۴', '1404/01/01', '1404/12/29', 1, new Date().toISOString(), new Date().toISOString());
...
if (!base) {
  ...
  const info = stmt.run('ستاد فرماندهی', 'base', new Date().toISOString(), new Date().toISOString());
...
if (!unit) {
  ...
  const info = stmt.run('معاونت دانش و پژوهش', base.id, 'unit', new Date().toISOString(), new Date().toISOString());
  unit = { id: info.lastInsertRowid };
}

// Ensure Template exists
let template1 = sqlite.prepare("SELECT id FROM templates WHERE title = ? LIMIT 1").get('گزارش پژوهشی') as any;
if(!template1) {
    const stmt = sqlite.prepare("INSERT INTO templates (type, title, is_active, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)");
    const info = stmt.run('پژوهشی', 'گزارش پژوهشی', 1, 0, new Date().toISOString(), new Date().toISOString());
    template1 = { id: info.lastInsertRowid };
}
let template2 = sqlite.prepare("SELECT id FROM templates WHERE title = ? LIMIT 1").get('سند راهبردی') as any;
if(!template2) {
    const stmt = sqlite.prepare("INSERT INTO templates (type, title, is_active, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)");
    const info = stmt.run('راهبردی', 'سند راهبردی', 1, 0, new Date().toISOString(), new Date().toISOString());
    template2 = { id: info.lastInsertRowid };
}

// Ensure Research Project Types
let rType = sqlite.prepare("SELECT id FROM research_project_types LIMIT 1").get() as any;
if(!rType) {
    sqlite.prepare("INSERT INTO research_project_types (name, created_at, updated_at) VALUES (?, ?, ?)").run('کاربردی', new Date().toISOString(), new Date().toISOString());
}

// Force delete existing trees named 'هوش مصنوعی' to allow re-running easily
sqlite.prepare("DELETE FROM issues").run();
sqlite.prepare("DELETE FROM research_items").run();
sqlite.prepare("DELETE FROM gaps").run();
sqlite.prepare("DELETE FROM tree_nodes WHERE tree_id IN (SELECT id FROM knowledge_trees WHERE name LIKE '%هوش مصنوعی%')").run();
sqlite.prepare("DELETE FROM knowledge_trees WHERE name LIKE '%هوش مصنوعی%'").run();
...
// Leaf
// Node1 requires 2 templates, but produced tree has only instances for 1, or nothing
...
const lNode2Prod = insertNode(prodTreeId, sbNode2Prod, 'L', 'موتور تحلیل متن فارسی', template1.id.toString(), "1,2"); // Has instances

const lNode3Req = insertNode(reqTreeId, sbNode2Req, 'L', 'سیستم پرسش و پاسخ', template2.id.toString());
const lNode3Prod = insertNode(prodTreeId, sbNode2Prod, 'L', 'سیستم پرسش و پاسخ', null, null); // Exists but no instances/templates met (Gap)

// Question (Q) - Level
const qNode1Req = insertNode(reqTreeId, lNode2Req, 'Q', 'چگونه دقت تحلیل متون تخصصی نظامی را افزایش دهیم؟');
const qNode2Req = insertNode(reqTreeId, lNode2Req, 'Q', 'بهترین معماری برای سرعت بخشیدن به ایندکس کردن چیست؟');

console.log("Comprehensive seed completed successfully! Trees are ready for testing.");

...
const oldTrees = sqlite.prepare("SELECT id FROM knowledge_trees WHERE name LIKE '%هوش مصنوعی%'").all();
if (oldTrees.length > 0) {
  const treeIds = oldTrees.map(t => t.id).join(',');
  const oldNodes = sqlite.prepare(`SELECT id FROM tree_nodes WHERE tree_id IN (${treeIds})`).all();
  if (oldNodes.length > 0) {
    ...
    sqlite.prepare(`DELETE FROM node_outputs WHERE node_id IN (${nodeIds})`).run();
  }
...
console.log("Trees seeded successfully! You can now check the required vs produced trees and gap analysis.");
